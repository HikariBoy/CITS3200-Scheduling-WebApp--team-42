...
function getUnavailabilityType(dateString) {
    const unavs = unavailabilityData.filter(unav => unav.date === dateString);
    if (unavs.length === 0) return null;
    
    // Check if we have both manual and auto-generated
    const hasManual = unavs.some(u => !u.is_auto_generated);
    const hasAuto = unavs.some(u => u.is_auto_generated);
    
    if (hasManual && hasAuto) {
        return 'mixed';  // Both types on same day
    } else if (hasAuto) {
        return 'auto';  // Only auto-generated
    } else {
        // Only manual - check if full or partial day
        return unavs[0].is_full_day ? 'full' : 'partial';
    }
}
...
function updateCalendarDisplay() {
    ...
    calendarDays.forEach(dayElement => {
        ...
        dayElement.classList.remove('unavailable', 'unavailable-full', 'unavailable-partial', 'auto-unavailable', 'mixed-unavailable');
...
function openUnavailabilityModal(date) {
    ...
    // Check if this date has auto-generated (scheduled) unavailability
    const hasAutoGenerated = unavailabilityData.some(u => u.date === date && u.is_auto_generated);
    const fullDayToggle = document.getElementById('full-day-toggle');
    const fullDayMessage = document.getElementById('full-day-disabled-message');
    
    // Disable full-day toggle if there are scheduled sessions (can't be unavailable full day if you have sessions)
    if (hasAutoGenerated) {
        if (fullDayToggle) {
            fullDayToggle.disabled = true;
            fullDayToggle.checked = false;
        }
        if (fullDayMessage) fullDayMessage.style.display = 'block';
    } else {
        if (fullDayToggle) fullDayToggle.disabled = false;
        if (fullDayMessage) fullDayMessage.style.display = 'none';
    }
...
function saveUnavailability() {
    ...
    if (isFullDay) {
        ...
        if (hasTimeRanges) {
            ...
            // Check for overlap with auto-generated (scheduled) unavailability
            // BUT only when CREATING new unavailability (not when editing/removing existing)
            const isCreatingNew = !editingId && !modal.dataset.editingIds;
            const autoGeneratedOnDate = unavailabilityData.filter(u => u.date === date && u.is_auto_generated);
            
            if (isCreatingNew && autoGeneratedOnDate.length > 0) {
                for (const newRange of timeRangesArray) {
                    for (const autoUnav of autoGeneratedOnDate) {
                        if (autoUnav.start_time && autoUnav.end_time) {
                            // Check if new range overlaps with scheduled session
                            if (timesOverlap(newRange.start_time, newRange.end_time, autoUnav.start_time, autoUnav.end_time)) {
                                alert(`Time range ${newRange.start_time}-${newRange.end_time} overlaps with your scheduled session:\n${autoUnav.reason || 'Scheduled commitment'}\n(${autoUnav.start_time} - ${autoUnav.end_time})\n\nYou cannot set unavailability during scheduled sessions.`);
                                return;
                            }
                        }
                    }
...
function updateRecentUnavailabilityList() {
    ...
    listContainer.innerHTML = recentUnavailability.map(unav => {
        ...
        const isAutoGenerated = unav.is_auto_generated;
        const itemClass = isAutoGenerated ? 'unavailability-item auto-generated' : 'unavailability-item';
        const icon = isAutoGenerated ? 'lock' : 'event_busy';
        const iconStyle = isAutoGenerated ? 'color: #f97316;' : '';
        
        // Show reason for auto-generated, or allow editing for manual
        let actionsHtml = '';
        if (isAutoGenerated) {
            actionsHtml = `
                <div class="item-reason" style="color: #f97316; font-size: 0.875rem; font-style: italic;">
...
function openUnavailabilityModalForEditMultiple(date, unavailabilityRecords) {
    ...
    setTimeout(() => {
        const container = document.getElementById('time-ranges-container');
        const fullDayToggle = document.getElementById('full-day-toggle');
        const fullDayContainer = document.querySelector('.full-day-container');
        if (!container) return;
        
        // Check if there are auto-generated entries (scheduled sessions)
        const hasAutoGenerated = unavailabilityRecords.some(u => u.is_auto_generated);
        const fullDayMessage = document.getElementById('full-day-disabled-message');
        ...
        // Check if ALL EDITABLE records are full-day (ignore auto-generated)
        const editableRecords = unavailabilityRecords.filter(u => !u.is_auto_generated);
        const allFullDay = editableRecords.length > 0 && editableRecords.every(u => u.is_full_day);
        
        if (allFullDay) {
            // All editable records are full-day - check the toggle
            if (fullDayToggle) fullDayToggle.checked = true;
            
            // Still show auto-generated time ranges even if manual is full-day
            const hasAutoGenerated = unavailabilityRecords.some(u => u.is_auto_generated);
            if (!hasAutoGenerated) {
                // No auto-generated - hide time ranges container
                container.style.display = 'none';
                const addBtn = document.getElementById('add-time-range');
                if (addBtn) addBtn.style.display = 'none';
                return;
            }
            // If there are auto-generated, continue to show them below
        }
